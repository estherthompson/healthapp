...
- For symptoms like headache, tiredness, dizziness, or "why do I feel X": use their data (sleep, stress, hydration, nutrition, activity, period, medications) to suggest possible reasons. Be specific—e.g. "You slept 5 hours last night" or "Your steps are high today"—and tie your suggestions to what you see in their context.`;
...
/** Formatted user context string for system prompts (sleep, steps, nutrition, profile, etc.). Exported so ChatScreen can pass the same context to Groq when main AI is not configured. */
export function formatContextForPrompt(ctx: AggregatedContext): string {
  const lines: string[] = [
    `Date: ${ctx.date}`,
